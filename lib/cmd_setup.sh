...
CFGEOF
    ...
    if [[ "$DEVICE_ROLE" == "hub" ]]; then
        # Copy hub dashboard
        if [[ -f "${GIH_REPO}/hub/index.html" ]]; then
            mkdir -p "${GIH_HOME}/hub"
            cp "${GIH_REPO}/hub/index.html" "${GIH_HOME}/hub/"
...
# ============================================================================
#  cmd_setup — Main entry point for the setup command
# ============================================================================
cmd_setup() {
    # Parse setup-specific arguments
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --lang)        FORCE_LANG="$2"; GIH_LANG="$2"; shift 2 ;;
            ...
            --help|-h)
                ...
                echo "  --dry-run            Check environment without installing"
                echo "  --skip-model         Skip model download"
                echo "  --device-role ROLE   Set role: hub | worker | power"
                echo "  --engine ENGINE      Model engine: ollama | llamacpp (default: ollama)"
                echo "  --model MODEL        Specific model ID to install"
    ...
    # Source additional libs needed for setup
    local lib_dir="${GIH_REPO}/lib"
    ...
    echo ""
    echo "  Quick start:"
    echo "    ollama run qwen3:4b          # Chat with local LLM"
    echo "    openclaw                      # Start AI agent"
    echo "    gih health                    # Run diagnostics"
    echo ""
    echo "  Tips:"
    echo "    ollama run qwen3:4b \"/no_think Hi\"  # Disable thinking (faster)"
    echo ""
